...
class MemoryManager:
    ...
    def __init__(self, db_path: str = "model_memory.db", pattern_extraction_frequency: int = 10, 
        ...
        if min_entries_for_guidance is None:
            min_entries_for_guidance = {
                "qwen_image": 50,      # Minimum entries for Qwen-Image guidance
                "qwen_image_edit": 10  # Minimum entries for Qwen-Image-Edit guidance
            }
    ...
    def get_workflow_guidance(self, prompt: str, model_type: str = "qwen_image") -> Dict[str, Dict[str, str]]:
        ...
        try:
            ...
            min_required = self.min_entries_for_guidance.get(model_type, 50)
...
def main(benchmark_name, human_in_the_loop, model_version, use_open_llm=False, open_llm_model="mistralai/Mistral-Small-3.1-24B-Instruct-2503", open_llm_host="0.0.0.0", open_llm_port="8000", calculate_latency=False, use_quantization=True, min_entries_for_guidance=None, gpu_id=0, total_gpus=1):
    ...
    if min_entries_for_guidance is None:
        min_entries_for_guidance = {
            "qwen_image": 50,      # Default minimum entries for Qwen-Image guidance
            "qwen_image_edit": 50  # Default minimum entries for Qwen-Image-Edit guidance
        }
    ...
    for benchmark_dir in benchmark_list:
        ...
        if total_gpus > 1:
            save_dir = os.path.join(base_save_dir, f"gpu__{gpu_id}")
        else:
        ...
        for idx, key in tqdm(enumerate(prompt_keys[start_idx:]), total=len(prompts) - start_idx):
            ...
            if "GenAIBenchmark" in str(benchmark_name):
                ...
                # Include GPU ID in the image index for GenAI benchmark
                if total_gpus > 1:
                    # Calculate a global index using the GPU's starting position and the local index
                    # We use start_idx_gpu (from multi-GPU split) + idx (local position in GPU's chunk)
                    global_idx = start_idx_gpu + idx
                    config.image_index = f"{global_idx:05d}"  # Format: 00123 (sequential across all GPUs)
                else:
                ...
                # Include GPU ID in the image index for DrawBench and other benchmarks
                if total_gpus > 1:
                    # Global index calculation: start_idx_gpu (GPU starting position) + idx (position within GPU's chunk)
                    global_idx = start_idx_gpu + idx
                    config.image_index = f"{global_idx:05d}"  # Format: 00123 (sequential across all GPUs)
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--min_qwen_image_entries', default=50, type=int, help='Minimum entries in qwen_image DB before guidance is used')
    parser.add_argument('--min_qwen_edit_entries', default=50, type=int, help='Minimum entries in qwen_image_edit DB before guidance is used')
